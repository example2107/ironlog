...
export default function App() {
  ...
  const [exercises, setExercises] = useState(() => {
    const saved = ls.get("wtp_ex", null);
    if (!saved || saved.length === 0) { ls.set("wtp_ex", DEFAULT_EXERCISES); return DEFAULT_EXERCISES; }
    return saved;
  });
  const [workouts, setWorkouts] = useState(() => ls.get("wtp_wo", []));
  ...
  const saveEx = useCallback(d => { setExercises(d); ls.set("wtp_ex", d); }, []);
  const saveWo = useCallback(d => { setWorkouts(d);  ls.set("wtp_wo", d); }, []);
  ...
  const commitMove = useCallback((exId, toGroup, afterId) => {
    setExercises(prev => {
      let moved = null;
      for (const g of prev) { const f = g.exercises.find(e => e.id === exId); if (f) { moved = f; break; } }
      if (!moved) return prev;
      const without = prev.map(g => ({ ...g, exercises: g.exercises.filter(e => e.id !== exId) }));
